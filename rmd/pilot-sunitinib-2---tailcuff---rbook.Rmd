...
data_dir <- "/Users/Nicholas/OneDrive - Tufts/phd/projects/vegfri/sunitinib/mouse-studies/data"
fn_name <- "Pilot 3 - Sunitinib oral gavage - MASTER.xlsx"
...
without_outliers_g <- ggplot(final_filtered_data, aes(x = `Specimen Name`, y = Systolic, color = group)) + 
  geom_boxplot() + 
  scale_color_manual(name = "group", values = levels(mark_outliers_df$color)) +
  geom_point(data = mark_outliers_df %>% filter(is.outlier), aes(x = `Specimen Name`, y = Systolic), color = "magenta", shape = 17, size = 3, alpha = 0.3) + 
  my_theme + 
...
ggplot(plot_time_series_df, 
  ...
  my_theme + 
  scale_fill_manual(values = c("gray", "#1e81b0", "#ed9942", "darkorange1")) + 
...
bt <- t.test(for_stats %>% filter(Phase == "baseline", group == "vehicle") %>% pluck("Systolic"),
       for_stats %>% filter(Phase == "baseline", group == "sunitinib") %>% pluck("Systolic")); # bt
vt <- t.test(for_stats %>% filter(Phase == "vehicle", group == "vehicle") %>% pluck("Systolic"),
       for_stats %>% filter(Phase == "vehicle", group == "sunitinib") %>% pluck("Systolic")); #vt
  
tt <- t.test(for_stats %>% filter(Phase == "treatment", group == "vehicle") %>% pluck("Systolic"),
       for_stats %>% filter(Phase == "treatment", group == "sunitinib") %>% pluck("Systolic")); #tt

tt2 <- t.test(for_stats %>% filter(Phase == "treatment 2x", group == "vehicle") %>% pluck("Systolic"),
       for_stats %>% filter(Phase == "treatment 2x", group == "sunitinib") %>% pluck("Systolic"));# tt2
